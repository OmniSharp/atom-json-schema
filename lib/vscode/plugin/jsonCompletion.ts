...
export class JSONCompletion {
    ...
    public doResolve(item: Suggestion) : Promise<Suggestion> {
    ...
    public doSuggest(document: Atom.TextEditor, textDocumentPosition: TextBuffer.Point, doc: Parser.JSONDocument): Promise<CompletionList> {

        let offset = document.getBuffer().characterIndexForPosition(textDocumentPosition);
        let node = doc.getNodeFromOffsetEndInclusive(offset);

        let currentWord = this.getCurrentWord(document, offset);
        let overwriteRange: TextBuffer.Range = null;
        ...
        return this.schemaService.getSchemaForResource(document.getURI(), doc).then((schema) => {
            ...
            if (node && node.type === 'object') {
                ...
                this.contributions.forEach((contribution) => {
                    let collectPromise = contribution.collectPropertySuggestions(document.getURI(), location, currentWord, addValue, isLast, collector);
                    if (collectPromise) {
            ...
            if (!node) {
                this.contributions.forEach((contribution) => {
                    let collectPromise = contribution.collectDefaultSuggestions(document.getURI(), collector);
                    if (collectPromise) {
                        collectionPromises.push(collectPromise);
                    }
                });
                ...
                if ((node.type === 'property') && offset > (<Parser.PropertyASTNode> node).colonOffset) {
                    ...
                    if (!valueNode || offset <= valueNode.end) {
                        ...
                        this.contributions.forEach((contribution) => {
                            let collectPromise = contribution.collectValueSuggestions(document.getURI(), location, parentKey, collector);
                            if (collectPromise) {
                                collectionPromises.push(collectPromise);
                            }
                        });
    ...
    private getSchemaLessValueSuggestions(doc: Parser.JSONDocument, node: Parser.ASTNode, offset: number, document: Atom.TextEditor, collector: ISuggestionsCollector): void {
    ...
    private getTextForMatchingNode(node: Parser.ASTNode, document: Atom.TextEditor): string {
        switch (node.type) {
            case 'array':
                return '[]';
            case 'object':
    ...
    private getCurrentWord(document: Atom.TextEditor, offset: number) {
        var i = offset - 1;
