...
export function format(document: Atom.TextEditor, range: TextBuffer.Range, options: { tabSize: number; insertSpaces: boolean; }): { range: TextBuffer.Range, text: string }[] {
    const documentText = document.getText();
    let initialIndentLevel: number;
    let value: string;
    let rangeOffset: number;
    if (range) {
        let startPosition = new Point(range.start.row, 0);
        rangeOffset = document.getBuffer().characterIndexForPosition(startPosition);

        let endOffset = document.getBuffer().characterIndexForPosition(new Point(range.end.row + 1, 0));
        let endLineStart = document.getBuffer().characterIndexForPosition(new Point(range.end.row, 0));
    ...
    function addEdit(text: string, startOffset: number, endOffset: number) {
        if (documentText.substring(startOffset, endOffset) !== text) {
            let replaceRange = new Range(document.getBuffer().positionForCharacterIndex(startOffset), document.getBuffer().positionForCharacterIndex(endOffset));
            editOperations.push({ range: replaceRange, text });
        }
    }
...
function computeIndentLevel(content: string, offset: number, options: { tabSize: number; insertSpaces: boolean; }): number {
    let i = 0;
    let nChars = 0;
